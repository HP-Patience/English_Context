      ...
      <div className="space-y-2">
        ...
        {stats?.groups?.map((g: any) => (
          <button
            ...
            className="flex w-full items-center justify-between rounded-xl border border-stone-200 bg-white px-4 py-3.5 text-left shadow-sm transition hover:border-stone-400 active:scale-[0.98] dark:border-stone-700 dark:bg-stone-900 dark:shadow-none dark:hover:border-stone-600"
          >
            <div>
              <p className="text-sm font-medium text-stone-800 dark:text-stone-200">{g.name}</p>
              <p className="mt-0.5 text-xs text-stone-400 dark:text-stone-500">{g.total} 词 · {g.learned} 已学</p>
            </div>
            <div className="flex items-center gap-2">
              <div className="h-1.5 w-16 overflow-hidden rounded-full bg-stone-100 dark:bg-stone-800">
                <div className="h-full rounded-full bg-stone-900 transition-all dark:bg-stone-100" style={{ width: `${g.total > 0 ? (g.learned / g.total * 100) : 0}%` }} />
              </div>
              <span className="text-xs text-stone-400 dark:text-stone-500">{g.learned}/{g.total}</span>
            </div>
          </button>
        ))}
      </div>
